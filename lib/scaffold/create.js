...
var BASE_PACKAGE = {
  description: 'A full Wwwcoin node build with Wwwcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'wwwcore-lib': '^' + bitcore.version,
    'wwwcore-node': version
  }
};

/**
 * Will create a directory and wwwcoin.conf file for Wwwcoin.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createWwwcoinDirectory(datadir, done) {
...
/**
 * Will create a base Wwwcore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The wwwcoin database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/wwwcore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Wwwcore Node directory, configuration file,
 * wwwcoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the wwwcore node configuration directory
 * @param {String} options.datadir - The path to the wwwcoin datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the wwwcore-node directory and configuration
    ...
    function(next) {
      // Setup the wwwcoin directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createWwwcoinDirectory(absDataDir, next);
      } else {
        next();
      }
    },

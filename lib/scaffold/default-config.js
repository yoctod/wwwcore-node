...
/**
 * Will return the path and default wwwcore-node configuration. It will search for the
 * configuration file in the "~/.wwwcoin" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.wwwcoin');
  var defaultConfigFile = path.resolve(defaultPath, './wwwcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['wwwcoind', 'web'];

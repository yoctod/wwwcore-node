...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for wwwcoind is necessary for this upgrade with the "reindex=1" wwwcoin.conf option. \n' +
                 'There are changes necessary in both wwwcoin.conf and wwwcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/bitpay/wwwcore-node/blob/wwwcoind/docs/upgrade.md \n');
        ...
        servicesConfig: {
          wwwcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/wwwcoind')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed wwwcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './wwwcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in wwwcore-node services directory
 ...
 * @param {Node} node - The Wwwcore Node instance
 */

...
var WwwcoinRPC = require('wwwcoind-rpc');
...
/**
 * Provides a friendly event driven API to wwwcoind in Node.js. Manages starting and
 * stopping wwwcoind as a child process for application support, as well as connecting
 * to multiple wwwcoind processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Wwwcoin(options) {
  if (!(this instanceof Wwwcoin)) {
    return new Wwwcoin(options);
  }
...
util.inherits(Wwwcoin, Service);

Wwwcoin.dependencies = [];

Wwwcoin.DEFAULT_MAX_TXIDS = 1000;
Wwwcoin.DEFAULT_MAX_HISTORY = 50;
Wwwcoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Wwwcoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Wwwcoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Wwwcoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
Wwwcoin.DEFAULT_SPAWN_STOP_TIME = 10000;
Wwwcoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
Wwwcoin.DEFAULT_REINDEX_INTERVAL = 10000;
Wwwcoin.DEFAULT_START_RETRY_INTERVAL = 5000;
Wwwcoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Wwwcoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Wwwcoin.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Wwwcoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Wwwcoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Wwwcoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Wwwcoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Wwwcoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Wwwcoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Wwwcoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Wwwcoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Wwwcoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Wwwcoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Wwwcoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Wwwcoin.prototype._initCaches = function() {
...
Wwwcoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Wwwcoin.prototype.getAPIMethods = function() {
  var methods = [
...
Wwwcoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'wwwcoind/' + name, 'total:', this.subscriptions[name].length);
};

Wwwcoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'wwwcoind/' + name, 'total:', this.subscriptions[name].length);
};

Wwwcoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'wwwcoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Wwwcoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'wwwcoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Wwwcoin.prototype.unsubscribeAddressAll = function(emitter) {
...
Wwwcoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Wwwcoin.DEFAULT_CONFIG_SETTINGS;
...
Wwwcoin.prototype._parseWwwcoinConf = function(configPath) {
  var options = {};
...
Wwwcoin.prototype._expandRelativeDatadir = function() {
...
Wwwcoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in wwwcoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in wwwcoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in wwwcoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './wwwcoin.conf');

  log.info('Using wwwcoin config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseWwwcoinConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseWwwcoinConf(networkConfigPath));
  }
...
Wwwcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of wwwcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of wwwcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of wwwcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to wwwcoind from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from wwwcoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from wwwcoind. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in wwwcoin.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that wwwcoind is undergoing a reindex. ' +
             ...
             'of wwwcore-node services will start.');
    node._reindex = true;
  }
};

Wwwcoin.prototype._resetCaches = function() {
...
Wwwcoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Wwwcoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Wwwcoin.prototype._initChain = function(callback) {
  var self = this;
...
Wwwcoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 5665
  };
  if (this.node.network === bitcore.Networks.testnet) {
    networkOptions.rpcport = 19665;
  }
  return networkOptions;
};

Wwwcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/wwwcoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/wwwcoin.conf';
    }
  }
  return networkPath;
};

Wwwcoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Wwwcoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('wwwcoind/hashblock', message.toString('hex'));
    }
  }

};

Wwwcoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Wwwcoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Wwwcoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Wwwcoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Wwwcoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('wwwcoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Wwwcoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('wwwcoind/rawtransaction', message.toString('hex'));
    }
...
Wwwcoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Wwwcoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Wwwcoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Wwwcoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Wwwcoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Wwwcoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Wwwcoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Wwwcoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Wwwcoin.prototype._stopSpawnedWwwcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/wwwcoind.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned wwwcoin process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean wwwcoin process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Wwwcoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedWwwcoin(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting wwwcoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Wwwcoin process unexpectedly exited with code:', code);
        log.warn('Restarting wwwcoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Wwwcoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn wwwcoind.'));
      }
...
Wwwcoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to wwwcoind.'));
    }
 ...
 */
Wwwcoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Wwwcoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
Wwwcoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Wwwcoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Wwwcoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Wwwcoin.prototype._getHeightRangeQuery = function(options, clone) {
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Wwwcoin.prototype._getConfirmationsDetail = function(transaction) {
...
Wwwcoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Wwwcoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Wwwcoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A wwwcoin transaction id
 * @param {Function} callback
 */
Wwwcoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Wwwcoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Wwwcoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Wwwcoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Wwwcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Wwwcoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Wwwcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Wwwcoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.getInfo = function(callback) {
  var self = this;
...
Wwwcoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wwwcoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('wwwcoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('wwwcoind process did not exit'));
      }
